...
class BittrexNotifier(NotifierUtils):
    ...
    def get_balance(self, epoch_time, currency):
        url = "https://api.bittrex.com/v3/balances/" + currency
        payload = "{}"
        payload_hash, signature = self.create_signature(epoch_time, url, 'GET', payload)
        headers = {
            'Api-Key': self.key,
    ...
    def get_rate(self, epoch_time, market):
    ...
    def bittrex(self, direction, market):
        ...
        prim_balance = self.get_balance(epoch_time, prim_currency)  # eth
        sec_balance = self.get_balance(epoch_time, sec_currency)    # usdt

        if direction == "SELL":
            order = self.order_bittrex(epoch_time, direction, prim_currency, prim_balance)
            print(order)
        elif direction == "BUY":
            rate = self.get_rate(epoch_time, self.market)
